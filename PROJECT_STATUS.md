...
5. ~~**Configurar preferências → preferências reais no restante do app**~~ **Persistência real concluída** (ver §3/§8 item 4); o "restante do app" que já lê Configurações hoje é só o financeiro (formas de pagamento ativas) — Lembretes ainda não consome.
...
4. ~~**Implementar Configurações real**~~ **CONCLUÍDO em 2026-07-30.** `configuracoes-repo.ts`/`configuracoes-actions.ts` + `ConfiguracoesProvider` implementados; formas de pagamento ativas já filtram os 3 modais financeiros. Horário/canal de lembretes e idioma padrão de mensagens ficam persistidos mas sem consumidor ainda — dependem do item 5.
5. **Lembretes: persistência real + geração automática** — agora destravado pelo item 4 (horário de aviso e canal preferido já estão em Configurações, prontos para ser lidos); é o módulo mais isolado que ainda está 100% mock.
