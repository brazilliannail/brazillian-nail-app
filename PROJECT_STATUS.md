...
Nestes 7 módulos, **nenhum dado fictício resta** e toda escrita passa por Server Actions que gravam de fato no SQLite.
...
6. **Home (`/`) real** — mesmo tratamento já aplicado ao Financeiro/Lembretes; menor valor de negócio que os itens acima, mas fecha o último resquício de dado fictício visível no dia a dia. **Próxima etapa recomendada.**
7. **Reengajamento de clientes** — feature nova e autocontida; agora que Configurações e Lembretes estão sólidos (inclusive `mensagens_log` de verdade), é o próximo módulo com maior valor de negócio depois de Home.
