...
- Certificação de manutenção: suíte ampliada para **96 testes**, incluindo inspeção preventiva do backup completo; certificação noturna com build adicionada; plano técnico de despesas documentado sem ativar o módulo nem alterar regras financeiras.
...
|---|---|---|
| **Clientes** (`clientes-repo.ts`, `clientes-actions.ts`) | Real (SQLite via Prisma) | CRUD manual verificado; histórico derivado do ledger; **desde 2026-08-02**, 9 testes de integração (7 originais: criação com contatos, validações de nome/telefone, sincronização de contato no update, toggle de status, busca por id; **+2 depois, no mesmo dia, em §11 item 2**: validação de telefone com mínimo de 7 dígitos — o "7 testes" era o estado antes dessa correção) |
| **Serviços** (`servicos-repo.ts`, `servicos-actions.ts`) | Real | CRUD manual verificado; **desde 2026-08-02**, 13 testes de integração (9 originais: validações de nome/categoria/preço/faixa variável/duração, update, toggle de status; **+4 depois, no mesmo dia, em §11 item 3**: CHECK constraints de preço/duração no banco — o "9 testes" era o estado antes dessa correção) |
| **Agenda** (`agenda-repo.ts`, `agenda-actions.ts`) | Real | Criar/editar/mudar status/reagendar, validação de conflito e expediente; **desde 2026-08-02**, 17 testes de integração (11 originais: expediente, conflito, cancelado não conflita, update/reagendar com sucesso e com erro; **+4 em §13 item 1**: reagendamento por status; **+2 novos e 1 reescrito em §13 item 3**: dias de funcionamento configuráveis e bloqueio de conflito desativável — o "11 testes" era o estado antes dessas duas decisões de negócio) |
| **Atendimentos + Pagamentos (ledger)** (`atendimentos-actions.ts`, `pagamentos-repo.ts`) | Real | Revisão técnica completa + 24 testes funcionais manuais (pagamento total/parcial/múltiplas formas/gorjeta/estorno/cancelamento/proteções); suíte automatizada `tests/integration/pagamentos-ledger.test.ts` com 9 testes de integração (6 originais do livro-razão + **3 em §11 item 4**: atomicidade da correção conjunta de lançamento); trava de edição financeira pós-pagamento coberta à parte por 6 testes em `tests/integration/atendimentos-edicao.test.ts` (ver §12) |
| **Financeiro/Dashboard** (`financeiro-repo.ts`, `financeiro-service.ts`) | Real | 24 testes funcionais manuais (D1–D4, consistência entre cards, estados vazios) + verificação visual no browser; **desde §13 item 2**, suíte automatizada `tests/integration/financeiro.test.ts` com 6 testes de integração (indicadores afetados pela regra de estornos) |
...
|---|---|---|
| **Reengajamento de clientes inativas** | Implementado: elegibilidade derivada dos atendimentos/agendamentos reais; filtro na tela de Clientes; ações persistidas de contatar, adiar e ignorar; reset automático ao iniciar novo atendimento | Validação visual local pendente por ausência das variáveis de autenticação/PostgreSQL no ambiente; lógica coberta por testes isolados |
...
4. ~~**Cliente inativa há 30+ dias → alerta de reengajamento → ação da profissional**~~ **Fechado em 2026-09-03**: elegibilidade calculada a partir de clientes ativas sem atendimento válido nos últimos 30 dias e sem agendamento futuro; ações contatada/adiada/ignorada persistidas; novo atendimento limpa a decisão anterior.
...
- ~~Horário de expediente configurado em Configurações não é lido pela validação real da Agenda (que usa constantes fixas 8h–18h).~~ **RESOLVIDO em 2026-08-02** (horário/dias de funcionamento/bloqueio de conflito implementados; `agendaPermitirEncaixe` — encaixe com confirmação manual — adiado para decisão futura de UX e removido da UI enquanto isso, ver §14) — ver §13 item 3.
...
As quatro decisões de negócio deixadas em aberto no §10 foram apresentadas ao usuário (regra atual, opções, vantagens/riscos e recomendação técnica para cada uma), **aprovadas nesta data** e **implementadas em 4 sessões separadas, uma por decisão, cada uma com sua própria bateria de testes/typecheck/lint e commit local** (ver item a item abaixo). Único ponto que ficou fora do escopo aprovado: `agendaPermitirEncaixe` (item 3), que exigiria uma UX de confirmação manual nova — sinalizado para decisão futura. **Nota de 2026-08-02 (sessão seguinte): esse corte não havia sido apresentado como opção antes da aprovação — foi uma decisão tomada durante a implementação. Como a tela de Configurações continuava exibindo o toggle e prometendo um comportamento ("confirmação manual" em sobreposições) que o sistema não executava, o toggle foi removido da UI nesta sessão até a UX de encaixe ser decidida — ver §14.**
...
3. **Horário de expediente em Configurações** (`agendaHorarioAbertura`/`agendaHorarioFechamento`/`agendaDiasFuncionamento`/`agendaBloqueioConflito`/`agendaPermitirEncaixe`) — **IMPLEMENTADO PARCIALMENTE em 2026-08-02.**
   ...
   - `agendaPermitirEncaixe` **não implementado nesta etapa** — sua semântica documentada na própria tela de Configurações ("sobreposições exigirão confirmação manual antes de salvar") pede um fluxo de confirmação novo (usuário tenta salvar um conflito, servidor recusa de forma distinta de um bloqueio duro, cliente oferece "salvar mesmo assim" e reenvia) que não existe hoje em nenhum lugar do app — e, com o valor padrão já `true`, implementá-lo pela metade quebraria a UX de conflito atual (o erro de conflito nunca teria como ser confirmado). Ficou de fora por ser uma decisão de UX nova, não coberta pelos objetivos desta etapa ("horários e dias de funcionamento" + eliminar constante fixa); registrado aqui para decisão explícita futura. **Correção em 2026-08-02 (sessão seguinte): o toggle correspondente na tela de Configurações foi removido — ver §14 — para o sistema parar de prometer um comportamento inexistente. `agendaPermitirEncaixe` continua fora do escopo atual, não é mais um item pendente desta etapa.**
...
## 14. Correção — toggle de encaixe com confirmação manual removido da UI (2026-08-02)

Durante a conferência final pré-`git push` da sessão do §13, o usuário identificou uma inconsistência entre o resumo apresentado ("as quatro decisões de negócio estão implementadas") e o estado real do item 3 (expediente): o campo `agendaPermitirEncaixe` era exibido como toggle funcional na tela de Configurações (`c.agenda.permitirEncaixe`/`permitirEncaixeDescricao`, prometendo "sobreposições exigirão confirmação manual antes de salvar"), mas nenhuma linha de `agenda-actions.ts` lê esse campo — o toggle não tinha nenhum efeito no comportamento real da Agenda.

Verificado nesta sessão que esse corte (deixar `agendaPermitirEncaixe` sem consumidor) **não constava como opção apresentada ao usuário antes da aprovação do §13** — foi uma decisão tomada durante a implementação do item 3 (commit `0fc6d09`) e só documentada depois no próprio PROJECT_STATUS.md. Ou seja, a UI continuava prometendo um comportamento que o sistema nunca chegou a executar, sem que isso tivesse sido uma escolha explícita do usuário.

**Correção aprovada e aplicada (sem implementar nenhuma regra de encaixe/confirmação manual nova):**
1. Toggle "Permitir encaixe com sobreposição mediante confirmação" removido de `src/app/configuracoes/page.tsx` (bloco `<Toggle checked={state.agenda.permitirEncaixeComConfirmacao} ...>` excluído; o toggle de `agendaBloqueioConflito`, que é funcional, permanece).
2. Chaves de tradução órfãs removidas de `src/lib/i18n.ts`: `agenda.permitirEncaixe`/`agenda.permitirEncaixeDescricao`, pt e en (não tinham mais nenhum outro consumidor).
3. Campo preservado sem alteração no schema/banco/estado: `agendaPermitirEncaixe` continua existindo em `configuracoes-repo.ts` (leitura/escrita da coluna) e `permitirEncaixeComConfirmacao` continua em `ConfiguracoesState`/`configuracoes-mock.ts` — nenhuma migration criada, nenhum dado perdido. O campo só deixou de ter uma superfície de UI que prometia um efeito inexistente; se o valor já estava salvo como `false` para alguém, nada muda para essa pessoa além do toggle sumir da tela.
4. Nenhuma regra de negócio de encaixe/confirmação manual foi implementada nesta correção — fica explicitamente para uma decisão futura de UX, a ser apresentada ao usuário como uma etapa nova (não uma retomada do §13 item 3, que agora é considerado concluído dentro do escopo que foi de fato aprovado).

**Testado:** suíte completa (`npm test`), `tsc --noEmit` e `npm run lint` rodados de novo após a correção — resultado no commit desta correção.

---

## 15. Verificação de consistência — contagens de teste (2026-08-02, sessão seguinte ao §14)

Antes de escolher a próxima etapa, o usuário pediu uma verificação de consistência somente leitura: rodar a suíte, `tsc --noEmit` e `npm run lint` de novo e comparar com os números que este documento já registrava.

- **`npm test`**: 69/69 passando, 7 arquivos — igual ao total já indicado no §4 item 1 e ao registro da sessão anterior (§13/§4). Nenhuma divergência entre o código e o "69" já documentado.
- **`tsc --noEmit`**: limpo.
- **`npm run lint`**: 0 erros, 8 warnings (mesmos de sempre: `_idPlaceholder` não usado em 6 arquivos + `f` não usado em `ClienteFormModal.tsx`).
- **O "57/57" do §12** não é uma divergência — é um marco histórico correto para o momento em que foi escrito (antes das 4 decisões de negócio do §13, implementadas depois, no mesmo dia, somando +12 testes: +4 em §13 item 1, +6 em §13 item 2, +2 líquidos em §13 item 3 — chegando a 69).
- **Inconsistência real encontrada e corrigida nesta sessão**: os sub-totais por arquivo listados no §1 e no §4 item 1 não tinham sido atualizados depois do §11 (Clientes "7", Serviços "9", ledger "6 cenários" — todos desatualizados; a soma dos sub-itens do §4 item 1 não batia com o "69" que o próprio parágrafo declarava) e `tests/integration/atendimentos-edicao.test.ts` (6 testes, existente desde o §12) não aparecia na enumeração do §4 item 1. Corrigido nesta sessão em ambas as seções, com nota explícita de qual número é o estado anterior e qual commit/item introduziu cada incremento.

**Nada além da documentação foi alterado**: nenhum código, migration, teste ou regra de negócio foi tocado nesta sessão — apenas texto do PROJECT_STATUS.md (§1 e §4 item 1, mais este §15).

---

## 16. Módulo Despesas e saldo operacional (2026-08-04)

Implementado o módulo projetado em `EXPENSES_DESIGN.md`, com as 7 decisões de negócio em aberto apresentadas e aprovadas pelo usuário antes da implementação, mais 10 ajustes obrigatórios adicionais definidos pelo usuário e 8 correções apontadas por revisão independente antes da publicação.

**Decisões de negócio aprovadas:** saldo usa só valores pagos/recebidos (não previstos, que aparecem separados); aluguel semanal em dia fixo configurável sem rateio de semana parcial; 1ª parcela de compra parcelada na data da compra; sem pagamento parcial (só pendente/pago/cancelado); categorias Aluguel/Contas fixas/Compras-Equipamentos/Serviços profissionais/Marketing/Outros; mudança de valor em recorrência afeta só lançamentos futuros; gorjeta fora do saldo do negócio (já era assim no resto do app).

**Ajustes obrigatórios do usuário:** valores em centavos inteiros (nunca `Float`); lançamento pago é imutável (sem cancelar/editar — correção só por novo lançamento de ajuste rastreável via `ajusta_lancamento_id`, mesmo padrão do estorno em `pagamentos`); backup em versão 2 com tratamento explícito da versão 1 anterior (`backup-validation.ts` aceita as duas); recorrência semanal gerada sob demanda com horizonte de 56 dias e teto de segurança de 60 lançamentos por chamada — nunca infinita; nenhuma exclusão definitiva pela UI, só `cancelado`/`cancelada`; saldo operacional = receitas recebidas − despesas pagas, nunca chamado de lucro; 1ª parcela na data da compra; gorjetas fora do saldo; nenhuma tabela existente (clientes/serviços/agenda/atendimentos/pagamentos) foi tocada; testes rodam só em banco descartável (PGlite em memória).

**Correções da revisão independente (antes da publicação):** FKs `lancamentos_despesa.despesa_id`/`ajusta_lancamento_id` trocadas de `CASCADE`/`SET NULL` para `RESTRICT`; CHECK do banco reforçado para exigir valor positivo em lançamento comum e `status = 'pago'` em todo ajuste; confirmação adicionada antes de cancelar uma despesa inteira ou um lançamento individual; correção de lançamento pago e alteração de valor semanal de recorrência (actions já existentes) ganharam interface própria; validação de data passou a checar calendário real (rejeita `2026-02-30`, por exemplo) e `dataEncerramento >= dataInicio`.

**Implementação:** duas tabelas novas (`despesas`, `lancamentos_despesa`, migration `20260804120000_despesas`), `despesas-mock.ts`/`despesas-repo.ts`/`despesas-actions.ts` (mesmo padrão repo+actions dos demais módulos), novo agregado e saldo operacional em `financeiro-service.ts` exibidos em `/financeiro` separadamente dos cards existentes, nova página `/despesas` com criação/listagem/filtros/cancelamento/correção, `DespesasProvider` alimentado pelo layout raiz, exportação JSON (backup versão 2) e CSV atualizadas. Nenhuma regra de negócio dos módulos existentes foi alterada.

**Testado:** `tests/integration/despesas.test.ts` com avulsa, parcelada (US$1.200/10x exato e caso com resto de centavos), recorrência semanal (horizonte limitado, idempotência, data de encerramento), pagamento, bloqueio de cancelar/editar lançamento pago, ajuste rastreável, cancelamento de despesa preservando pagos, filtros, saldo, exportação, restrições de FK/CHECK no banco e validação de datas de calendário. Suíte completa: **128/128 testes passando**, `tsc --noEmit` e `npm run lint` limpos, `npm run build` validado (localmente, com variáveis de ambiente fictícias, já que sessões de desenvolvimento não têm acesso às credenciais reais do Neon/produção).

---

## 17. Reengajamento de clientes (2026-09-03)

Implementada a regra documentada em `DATABASE_DESIGN.md` §4.1.1. Uma cliente aparece no filtro **Reengajar** quando está ativa, não possui atendimento válido nos últimos 30 dias, não possui agendamento futuro que ocupe agenda e sua decisão atual é `nenhum` ou um adiamento já vencido. A identificação continua derivada dos dados reais e nunca é armazenada.

A ficha da cliente permite registrar **contatada**, **adiada** (com data futura obrigatória) ou **ignorada**, com observação opcional. A action valida novamente a elegibilidade dentro da transação para impedir gravação sobre estado desatualizado. Ao criar um atendimento avulso ou iniciar um atendimento pela Agenda, qualquer decisão anterior é limpa na mesma transação.

Testes novos em `tests/integration/reengajamento.test.ts`: elegibilidade sem histórico, exclusão por atendimento recente/agendamento futuro, persistência das três decisões, validação da data de adiamento e reset ao iniciar novo atendimento. Suíte completa: **244/244 testes passando**, 23 arquivos; typecheck, lint e build aprovados. Build executado somente com variáveis fictícias temporárias, sem conexão com produção. Validação visual local continua pendente porque o checkout não possui as variáveis de autenticação e PostgreSQL necessárias para abrir o app.

---

## 18. Reconfirmação, ações rápidas e fechamento do checkpoint (2026-09-04)

- Editar data, horário ou serviço de um agendamento confirmado agora volta o status para **aguardando confirmação**; alterações não relevantes preservam o status. O fluxo de reagendamento mantém a mesma regra e possui cobertura específica em `tests/integration/agenda.test.ts`.
- A ficha da cliente ganhou ações rápidas reais para **Agendar** e **Iniciar atendimento**, com a cliente já selecionada e atualização da ficha sem depender de navegação manual.
- Aberturas de WhatsApp/SMS fora de Lembretes (Agenda e ficha da cliente) agora registram somente uma mensagem **preparada** em `mensagens_log`, nunca presumem envio e preservam canal, idioma, contato e texto. A geração sequencial ganhou até três tentativas quando duas abas disputam o mesmo `MAX+1`; erros diferentes de colisão continuam sendo propagados.
