...
1. ~~**Nenhum teste automatizado persistido no repositório.**~~ **PARCIALMENTE RESOLVIDO em 2026-07-30, AMPLIADO em 2026-08-02.** Vitest configurado (`vitest.config.ts`, `npm test`), com banco SQLite de teste isolado recriado do zero a cada run (`tests/setup/global-setup.ts`, migrations aplicadas de verdade — não uma cópia). Cobertura hoje: `tests/integration/pagamentos-ledger.test.ts` (6 cenários do livro-razão), `tests/integration/lembretes.test.ts` (9 cenários), `tests/integration/agenda.test.ts` (agora 15 cenários, +4 de reagendamento por status — ver §13 item 1), `tests/integration/clientes.test.ts` (7 cenários), `tests/integration/servicos.test.ts` (9 cenários) e, desde §13 item 2, `tests/integration/financeiro.test.ts` (6 cenários, funções puras de `financeiro-service.ts`, sem I/O) — **67 testes no total, todos passando**. Novo helper `tests/helpers/agenda-fixtures.ts` gera datas únicas por teste para os cenários de conflito de horário não colidirem entre si nem com as datas fixas usadas por Ledger/Lembretes (banco de teste é compartilhado entre todas as suítes). **Ainda falta**: `financeiro.test.ts` cobre só os indicadores afetados pela regra de estornos (ver §13 item 2) — o resto do Dashboard Financeiro (comparação de período, listas de pagamentos/pendências, forma de pagamento) e Configurações continuam sem teste automatizado (só verificação manual); nenhuma suíte de componente/UI existe; sem CI (ver item 7), a suíte só roda quando alguém lembra de rodar `npm test` manualmente.
