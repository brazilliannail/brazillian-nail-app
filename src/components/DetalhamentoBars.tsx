...
import { VALOR_OCULTO } from "@/components/FinancialVisibilityProvider";
...
type DetalhamentoBarsProps = {
  ...
  /** Quando `false` ("Ocultar valores" ativo), mascara o valor e neutraliza a largura relativa
   * das barras — resolvido pelo chamador (`useFinancialVisibility`), mesmo padrão desta função
   * de resto: nenhum estado/contexto é lido aqui além de idioma. */
  visible: boolean;
};
...
export function DetalhamentoBars({ dados, visible }: DetalhamentoBarsProps) {
  const { t } = useLanguage();
